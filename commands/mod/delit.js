...
function ensureDb(client) {
  ...
  db.prepare(
    `CREATE TABLE IF NOT EXISTS del_access (
...
// Fixed usage embed – correct template strings
function usageEmbed(prefix) {
  return makeEmbed(
    ...
    [
      ...
      `\`${prefix}delit\` (reply to a message)`,
      '',
      '**Manage delete access:**',
      `\`${prefix}delit access add @user|userID|username\``,
      `\`${prefix}delit access remove @user|userID|username\``,
      `\`${prefix}delit access list\``,
    ].join('\n')
  );
}
...
module.exports = {
  ...
  async execute(client, message, args) {
    console.log('[DELIT] Command executed'); // Debug – remove after confirming it works
    ...
    if (!botMember?.permissions?.has(PermissionFlagsBits.ManageMessages)) {
      return message.reply({
        embeds: [
          makeEmbed('#ef4444', 'Delit Failed', 'I need **Manage Messages** permission.')
        ]
      });
    }
    ...
    if (sub === 'access') {
      if (!canManageDelAccess(client, message)) {
        return message.reply({
          embeds: [
            makeEmbed('#ef4444', 'Access Denied', 'Only the bot owner or server owner can manage delete access.')
          ]
        });
      }
      ...
      if (action === 'list') {
        ...
        if (!rows.length) {
          return message.reply({
            embeds: [
              makeEmbed('#3b82f6', 'Delete Access List', 'No users have delete access in this server.')
            ]
          });
        }
        ...
        return message.reply({
          embeds: [makeEmbed('#3b82f6', 'Delete Access List', clean)]
        });
      }
      ...
      if (!target) {
        return message.reply({
          embeds: [
            makeEmbed('#f59e0b', 'Delit Access Failed', 'Provide a valid user mention, ID, or exact username.')
          ]
        });
      }
      ...
      if (target.bot) {
        return message.reply({
          embeds: [
            makeEmbed('#f59e0b', 'Delit Access Failed', 'Bots do not need delete access.')
          ]
        });
      }
      ...
      if (action === 'add') {
        ...
        return message.reply({
          embeds: [
            makeEmbed('#22c55e', 'Access Added', `**${target.tag}** can now use \`${prefix}delit\` in this server.`)
          ]
        });
      }
      ...
      return message.reply({
        embeds: [
          makeEmbed('#ef4444', 'Access Removed', `**${target.tag}** can no longer use \`${prefix}delit\` in this server.`)
        ]
      });
    }

    // Normal delit – delete replied message
    if (!canUseDelIt(client, message)) {
      return message.reply({
        embeds: [
          makeEmbed('#ef4444', 'Delit Failed', 'You do not have permission to use this command.')
        ]
      });
    }
    ...
    try {
      ...
      return message.reply({
        embeds: [
          makeEmbed('#ef4444', 'Delit Failed', 'Failed to delete the replied message.')
        ]
      });
    }
  },
