...
function resolveTargetId(message, raw) {
  ...
  // try guild members by display name / username (case-insensitive)
  if (message.guild) {
...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!message.member.permissions.has(PermissionFlagsBits.ManageMessages) &&
      ...
      return; // silent deny as requested elsewhere — but you can change to reply
    }
    ...
    if (!targetId) return message.reply('Could not find that user (use mention / ID / exact username).');
    ...
    // Build embed
    const userTag = (message.mentions.users.first() || message.guild.members.cache.get(targetId)?.user) || { tag: `ID: ${targetId}`, id: targetId };
    const embed = new EmbedBuilder()
      .setTitle('🗒️ Note Added')
      .setColor('#f59e0b')
      .addFields(
        { name: 'User', value: `${userTag.tag || `ID: ${targetId}`} (ID: ${targetId})`, inline: false },
