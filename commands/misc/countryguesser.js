...
module.exports = {
  name: 'countryguesser',
  description: 'Starts a game where you guess the country from its capital',
  category: 'games',
  usage: '!countryguesser',
  async execute(client, message, args) {
    const selected = countries[Math.floor(Math.random() * countries.length)];
    const embed = new EmbedBuilder()
      .setTitle('🌍 Country Guesser')
      .setDescription(`Guess the country of this capital: **${selected.capital}**`)
      .setColor('#22c55e')
      .setFooter({ text: 'Type your answer in chat!' });

    await message.channel.send({ embeds: [embed] });

    const filter = m => m.content.toLowerCase() === selected.country.toLowerCase();
    const collector = message.channel.createMessageCollector({ filter, time: 30000, max: 1 });

    collector.on('collect', m => {
      m.channel.send(`🎉 Congrats ${m.author}, you guessed it! The country was **${selected.country}**.`);
    });

    collector.on('end', collected => {
      if (collected.size === 0) {
        message.channel.send(`⏰ Time's up! The country was **${selected.country}**.`);
      }
    });
  }
};