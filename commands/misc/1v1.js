...
function canUseAdmin1v1(client, message) {
  return (
    message.guild &&
    (
...
function calcRP(winnerRp, loserRp, winStreak) {
  ...
  if (rankDiff > 0) {
    gainBase = Math.min(60, 30 + rankDiff * 10);
    lossBase = Math.max(10, 20 - rankDiff * 3);
  } else if (rankDiff < 0) {
    gainBase = Math.max(10, 30 + rankDiff * 8);
    lossBase = Math.min(40, 20 - rankDiff * 5);
  }
...
async function getDisplayName(guild, user) {
  try {
    return (await guild.members.fetch(user.id)).displayName;
  } catch {
...
async function buildBattleImage(challengerUser, opponentUser, guild) {
  ...
  const positions = [
    [Math.round(W * 0.25), avatarY, challengerUser, challName],
...
async function handleRank(message, args) {
  ...
  if (!target && args[1]) {
    target = await guild.members.fetch(args[1].replace(/\D/g, '')).then(m => m?.user).catch(() => null);
  }
...
async function handleAdminResetCD(message, args) {
  if (!canUseAdmin1v1(message.client, message)) {
