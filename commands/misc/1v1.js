...
db.prepare(  INSERT INTO inventory (user_id, animal, amount) VALUES (?, ?, ?)   ON CONFLICT(user_id, animal) DO UPDATE SET amount = amount + excluded.amount  ).run(userId, animal, amount);
}
}
}

const upsertInventory = db.prepare(  INSERT INTO inventory (user_id, animal, amount) VALUES (?, ?, ?)   ON CONFLICT(user_id, animal) DO UPDATE SET amount = amount + excluded.amount  );
