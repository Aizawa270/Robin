...
function ensureWatchwordDB(client) {
  ...
  client.watchwordDB.prepare(`
    CREATE TABLE IF NOT EXISTS watchword_disabled_channels (
      guild_id TEXT NOT NULL,
...
// ---------- Bot owner helpers (fixed config.ownerIds array) ----------
...
// Fixed word matching with template literal
...
// Fixed help embed – proper template string
function buildHelpEmbed(prefix) {
  return makeEmbed(
    '#ec4899',
    'Watchword Commands',
    [
      `\`${prefix}watchword add <word>\``,
      `\`${prefix}watchword remove <word>\``,
      `\`${prefix}watchword list\``,
      `\`${prefix}watchword disable <#channel|channel_id|channel_name>\``,
      `\`${prefix}watchword enable <#channel|channel_id|channel_name>\``,
      `\`${prefix}watchword listdisabled\``,
      '',
...
module.exports = {
  ...
  // This function is called from your messageCreate event
