...
// Create tables matching the new per-server schema
db.prepare(`
...
module.exports = (client) => {
  setInterval(async () => {
    ...
    for (const [guildId, guild] of client.guilds.cache) {
      ...
      // Get all birthdays from this guild for today
      ...
      for (const row of birthdayRows) {
        ...
        // Announce in the guild's birthday channel
        await channel.send({
          content: `<@${user.id}>`,
          embeds: [embed]
        }).catch(() => {});

        // Update last_sent_year for this specific guild/user
        db.prepare(`
          UPDATE birthdays
          SET last_sent_year = ?
