...
// in-memory state for live pending actions: messageId -> {guildId, targetUserId, status}
...
function buildAlertEmbed(guild, targetUser, matchedWord, requirementRoleId) {
  const embed = new EmbedBuilder()
    ...
    .setDescription([
      ...
      `➤  **Trigger:** \`${matchedWord}\``,
      `➤  **Channel:** <#${guild.id ? guild.systemChannelId || '' : ''}>`, // we keep it simple
...
// helper: permission test for staff
...
// send automod alert (ghost ping method)
async function sendAutomodAlert(client, guild, targetUser, matchedWord, requirementRoleId = null) {
  ...
  if (!channelId) return; // no alert channel set

  const channel = await client.channels.fetch(channelId).catch(() => null);
  if (!channel) return;

  // Build list of mentions from DB for this guild
  ...
  // send message with content mentions so they get notified, then remove content quickly (ghost ping)
  ...
  // remove the mentions from the message content so mentions don't show (ghost ping already delivered)
  setTimeout(() => {
    try { sent.edit({ content: '\u200b' }); } catch {}
  }, 1200);

  // attach collector for buttons — times out after 30 minutes
  ...
  collector.on('collect', async (interaction) => {
    ...
    if (custom === `am_warn:${sent.id}`) {
      // first staff to click opens a modal to get the reason
      state.handled = true; // reserve the action
      pendingActions.set(sent.id, state);
      ...
      // modal submit handled in global interactionCreate below
      return;
    }
...
// export init -> attach interaction handler for modals + ban confirm
function initAutomod(client) {
  ...
  // interactionCreate handler for modal submit and ban confirm
  client.on('interactionCreate', async (interaction) => {
    if (interaction.isModalSubmit()) {
      ...
      if (custom && custom.startsWith('am_warn_modal:')) {
        ...
        // edit original alert embed to show warned
        ...
        await interaction.reply({ content: `User warned and reason saved.`, ephemeral: true });
      }
      return;
    }
  ...
  // main checkMessage function to be called from your message handler
  async function checkMessage(client, message) {
    try {
      ...
      for (const w of hardWords) {
        if (!w) continue;
        if (content.includes(w.toLowerCase())) {
          // delete message, send automod alert
          await message.delete().catch(() => {});
      ...
      // check invite links (simple regex)
  ...
  // attach the function to client so your command handler can call it
